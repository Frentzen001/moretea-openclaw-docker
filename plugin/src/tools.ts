...
import type { RosbridgeClient } from "./rosbridge.js";
...
export type EyeExpressionName = keyof typeof EYE_EXPRESSION_MAP;
...
function buildTwist(linear_x: number, linear_y: number, angular_z: number) {
  return {
...
function normalizeEyeExpressionName(value: string): EyeExpressionName | null {
  const normalized = value.trim().toLowerCase();
  const aliases: Record<string, EyeExpressionName> = {
    neutral: "neutral",
    normal: "neutral",
    calm: "neutral",
    happy: "happy",
...
function extractOdomPosition(msg: unknown): { x: number; y: number } | null {
  const pose = (msg as Record<string, unknown>)?.["pose"] as Record<string, unknown> | undefined;
  const nestedPose = pose?.["pose"] as Record<string, unknown> | undefined;
  const position = nestedPose?.["position"] as Record<string, unknown> | undefined;
  const x = position?.["x"];
  const y = position?.["y"];
...
function extractOdomYaw(msg: unknown): number | null {
  const pose = (msg as Record<string, unknown>)?.["pose"] as Record<string, unknown> | undefined;
  const nestedPose = pose?.["pose"] as Record<string, unknown> | undefined;
  const orientation = nestedPose?.["orientation"] as Record<string, unknown> | undefined;
...
export async function cmdVel(
  ...
  if (!ros.connected) return fail("Not connected to rosbridge");
  ...
  try {
    ...
    while (Date.now() < endTime) {
      ...
      ros.publish(
        ...
        buildTwist(linear_x * scale, linear_y * scale, effectiveAngularZ * scale)
      );
...
export async function moveLinearDistance(
  ros: RosbridgeClient,
  cmdVelTopic: string,
  odomTopic: string,
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      const desiredSpeed = Math.min(
        ...
        Math.max(0.03, remainingDistance * 1.2)
      );
      ...
      ros.publish(
        cmdVelTopic,
        "geometry_msgs/msg/Twist",
        buildTwist(commandedSpeed * direction, 0, 0)
      );
    ...
    ros.publish(cmdVelTopic, "geometry_msgs/msg/Twist", zeroTwist());
    return fail(
      `Timed out before reaching target distance. Target=${targetDistance}m, traveled=${lastDistance.toFixed(3)}m`
    );
  } catch (e) {
    ros.publish(cmdVelTopic, "geometry_msgs/msg/Twist", zeroTwist());
    return fail(String(e));
...
export async function turnAngle(
  ros: RosbridgeClient,
  cmdVelTopic: string,
  odomTopic: string,
  angleDegrees: number,
  ...
  if (
    !Number.isFinite(angularSpeedRadPerSec) ||
  ...
  try {
    ...
    while (Date.now() < deadline) {
      ...
      const desiredAngularSpeed = Math.min(
        angularSpeedRadPerSec,
        Math.max(0.08, remainingRadians * 1.8)
      );
      commandedAngularSpeed = moveTowards(
      ...
      ros.publish(
        cmdVelTopic,
        "geometry_msgs/msg/Twist",
        buildTwist(0, 0, direction * commandedAngularSpeed * angularSign)
      );
    ...
    ros.publish(cmdVelTopic, "geometry_msgs/msg/Twist", zeroTwist());
    return fail(
      `Timed out before reaching target angle. Target=${Math.abs(angleDegrees)}deg, turned=${(turnedRadians * 180 / Math.PI).toFixed(1)}deg`
    );
  } catch (e) {
    ros.publish(cmdVelTopic, "geometry_msgs/msg/Twist", zeroTwist());
    return fail(String(e));
...
export async function executeMotionSequence(
  ros: RosbridgeClient,
  cmdVelTopic: string,
  odomTopic: string,
  angularSign: number,
  ...
  for (const [index, step] of steps.entries()) {
    ...
    if (step.action === "move") {
      ...
      stepResult = await moveLinearDistance(
        ros,
        cmdVelTopic,
        odomTopic,
        step.distance_m as number,
        step.speed_mps ?? 0.15
      );
      ...
      stepResult = await turnAngle(
        ros,
        cmdVelTopic,
        odomTopic,
        step.angle_deg as number,
        step.angular_speed_rad_s ?? 0.35,
        angularSign
      );
...
export async function publishStringTopic(
  ros: RosbridgeClient,
  topic: string,
  value: string,
  repeatHz = 0,
  ...
  if (!ros.connected) return fail("Not connected to rosbridge");
  if (!value.trim()) return fail("value must be a non-empty string");
...
export async function setFaceDirectionY(
  ros: RosbridgeClient,
  value: string,
  topic = "/face_direction_y",
  repeatHz = 0,
  durationMs = 0
): Promise<ToolResult> {
...
export async function setEyeExpression(
  ros: RosbridgeClient,
  input: string | number,
  ...
  let emotion: EyeExpressionName | null = null;

  if (typeof input === "number") {
    code = input;
    emotion =
      (Object.entries(EYE_EXPRESSION_MAP).find(([, mappedCode]) => mappedCode === input)?.[0] as
        | EyeExpressionName
        | undefined) ?? null;
  } else {
...
export interface NavGoalHandle {
  goalId: string;
  accepted: boolean;
  completionPromise: Promise<ToolResult>;
}

// Active navigation goals — keyed by goalId.
// The plugin checks this map to cancel on interrupt.
export const activeNavGoals = new Map<string, { cancel: () => void }>();
 ...
 */
export function navigateToPoseAsync(
  ros: RosbridgeClient,
  x: number,
  y: number,
  yaw_degrees = 0,
  ...
  if (!ros.connected) return Promise.resolve(fail("Not connected to rosbridge"));
  ...
  ros.sendActionGoal(
    ...
    goal,
    undefined // feedback handler optional — omit to keep context clean
  ).then((result) => {
    activeNavGoals.delete(goalId);
    if (!cancelled) onComplete?.(ok(result));
  }).catch((e) => {
    activeNavGoals.delete(goalId);
    if (!cancelled) onComplete?.(fail(String(e)));
  });
...
export async function navigateToPose(
  ros: RosbridgeClient,
  x: number,
  y: number,
  yaw_degrees = 0,
  frame_id = "map",
  onFeedback?: (fb: unknown) => void
): Promise<ToolResult> {
  if (!ros.connected) return fail("Not connected to rosbridge");
...
export async function subscribeTopic(
  ...
  if (!ros.connected) return fail("Not connected to rosbridge");
  try {
    const raw = await ros.subscribeOnce(topic, msgType) as Record<string, unknown>;
...
function compactSummary(
  ...
  if (msgType.includes("BatteryState")) {
    const pct = Math.round((msg["percentage"] as number) * 100);
    const v = (msg["voltage"] as number)?.toFixed(1);
  ...
  if (msgType.includes("Odometry") || topic.includes("odom")) {
    ...
    if (pos) {
      return `Position: x=${(pos["x"] as number)?.toFixed(2)}, y=${(pos["y"] as number)?.toFixed(2)}`;
    }
  }

  if (msgType.includes("PoseWithCovarianceStamped") || topic === "/pose") {
    const poseWithCov = msg["pose"] as Record<string, unknown>;
    const pose = poseWithCov?.["pose"] as Record<string, unknown>;
    const pos = pose?.["position"] as Record<string, unknown>;
    if (pos) {
      return `Pose: x=${(pos["x"] as number)?.toFixed(2)}, y=${(pos["y"] as number)?.toFixed(2)}`;
    }
  }
  ...
  if (msgType.includes("NavSatFix")) {
    return `GPS: ${msg["latitude"]}, ${msg["longitude"]} (status ${(msg["status"] as Record<string, unknown>)?.["status"]})`;
  }
...
export async function listTopics(ros: RosbridgeClient): Promise<ToolResult> {
  if (!ros.connected) return fail("Not connected to rosbridge");
  try {
...
// ── Camera snapshot ────────────────────────────────────────────────────────
// ROS2 sensor_msgs/Image contains raw pixel data. We re-encode it to PNG
// base64 so the LLM (if vision-capable) can describe what it sees.
// If no vision model is configured, returns metadata only.
...
export async function cameraSnapshot(
  ...
  try {
    const compressedMsg = await ros.subscribeOnce(
      compressedTopic,
    ...
    try {
      const rawMsg = await ros.subscribeOnce(
        imageTopic,
      ...
      return ok({
        ...
        error:
          `Read raw image metadata from ${imageTopic}, but a compressed image stream was not available. ` +
          `Compressed attempt failed: ${compressedError}`,
      });
    } catch (rawError) {
      return fail(
        `Could not read image from ${imageTopic} or ${compressedTopic}: ${rawError}`
      );
    }
  }
}
