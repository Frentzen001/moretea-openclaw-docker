...
export class RosbridgeClient {
  ...
  >();
  private pendingActionFeedback = new Map<
    string,
    { onFeedback?: (fb: unknown) => void; resolve: (r: unknown) => void; reject: (e: Error) => void }
  >();
  ...
  connect(): Promise<void> {
    return new Promise((resolve, reject) => {
      const timer = setTimeout(
        () => reject(new Error(`rosbridge connection timed out (${this.config.url})`)),
  ...
  private send(payload: object): void {
    if (!this.connected) throw new Error("Not connected to rosbridge");
  ...
  subscribeOnce(topic: string, msgType: string, timeoutMs = 3000): Promise<unknown> {
    return new Promise((resolve, reject) => {
  ...
  private unsubscribe(topic: string, _id: string): void {
  ...
  async getParam(node: string, param: string): Promise<unknown> {
  ...
  async setParam(node: string, param: string, value: unknown): Promise<void> {
