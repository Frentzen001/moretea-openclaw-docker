...
export async function register(api: PluginAPI): Promise<void> {
  const config = api.config as {
    ...
    odomTopic?: string;
    angularSign?: number;
    faceDirectionYTopic?: string;
    faceDirectionYDownValue?: string;
    faceDirectionYUpValue?: string;
    faceDirectionYCenterValue?: string;
    faceDirectionYRepeatHz?: number;
    faceDirectionYDurationMs?: number;
  };
  ...
  let ros: RosbridgeClient | null = null;

  async function getRos(): Promise<RosbridgeClient> {
  ...
  api.tools.register({
    name: "ros2_move",
    description:
      "Send velocity commands to the robot. Use for direct movement: forward, backward, turn, stop. " +
  ...
  api.tools.register({
    ...
    description:
      "Move the robot a measured linear distance using odometry feedback. " +
  ...
  api.tools.register({
    ...
    description:
      "Execute a sequence of motion steps serially on the robot. " +
    ...
    handler: async ({ steps }) => {
      try {
        const r = await getRos();
        return T.executeMotionSequence(
          r,
          cmdVelTopic,
          odomTopic,
          angularSign,
          steps as T.MotionSequenceStep[]
        );
      } catch (e) {
        return { success: false, error: String(e) };
      }
  ...
  api.tools.register({
    ...
    description:
      "Publish a face/neck vertical direction command to /face_direction_y. " +
    ...
    parameters: {
      ...
      properties: {
        direction: {
          ...
          enum: ["up", "down", "center"],
          description:
            "High-level face direction. Uses the robot's configured symbol mapping for /face_direction_y.",
        },
        value: {
          type: "string",
          description:
            "String to publish to /face_direction_y. Common robot-specific values are '-', '+', or '0'.",
        },
      },
    },
    handler: async ({ direction, value }) => {
      try {
        ...
        return T.setFaceDirectionY(
          r,
          resolvedValue,
          faceDirectionYTopic,
          faceDirectionYRepeatHz,
          faceDirectionYDurationMs
        );
      } catch (e) {
        return { success: false, error: String(e) };
      }
  ...
  api.tools.register({
    ...
    description:
      "Set the robot eye expression on /eye_expression. " +
    ...
    parameters: {
      ...
      properties: {
        emotion: {
          type: "string",
          description:
            "Emotion name. Supported: neutral, happy, sad, angry, confused, shocked, love, shy.",
        },
        code: {
          type: "number",
  ...
  api.tools.register({
    name: "ros2_navigate",
    description:
      "Send an autonomous navigation goal to Nav2. Returns immediately once the goal is accepted — " +
    ...
    handler: async ({ x, y, yaw_degrees = 0, frame_id = "map", location_name = "destination" }) => {
      try {
        ...
        return T.navigateToPoseAsync(r, x, y, yaw_degrees, frame_id, (result) => {
          ...
          if (!result.success) {
            api.logger.warn(
              `[rosclaw] Navigation to ${location_name} failed: ${result.error}. ` +
  ...
  api.tools.register({
    ...
    description:
      "Cancel the current Nav2 navigation goal. Call this if the user says 'stop', " +
  ...
  api.tools.register({
    ...
    description:
      "Read the latest message from any ROS2 topic. Use for sensor data, battery state, robot pose, etc.",
    parameters: {
      type: "object",
      properties: {
        topic: {
        ...
        msg_type: {
          type: "string",
          description:
            "ROS2 message type, e.g. sensor_msgs/msg/BatteryState. " +
  ...
  api.tools.register({
    ...
    description:
      "Call any ROS2 service. Use for discrete actions that return a result: " +
  ...
  api.tools.register({
    ...
    description:
      "Capture a single image frame from the robot's camera. " +
